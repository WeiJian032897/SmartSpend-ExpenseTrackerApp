...
class EditPlannedPaymentScreen extends StatefulWidget {
  final PlannedPaymentModel payment;
  
  const EditPlannedPaymentScreen({super.key, required this.payment});

  @override
  State<EditPlannedPaymentScreen> createState() => _EditPlannedPaymentScreenState();
}

class _EditPlannedPaymentScreenState extends State<EditPlannedPaymentScreen> {
  late TextEditingController _amountController;
  late String _selectedCurrency;
  late String _paymentName;
  late String _selectedAccount;
  late String _selectedCategory;
  late String _confirmationType;
  ...
  void initState() {
    super.initState();
    // Initialize with existing payment data
    _amountController = TextEditingController(text: widget.payment.amount.toString());
    _selectedCurrency = widget.payment.currency;
    _paymentName = widget.payment.paymentName;
    _selectedAccount = widget.payment.account;
    _selectedCategory = widget.payment.category;
    _confirmationType = widget.payment.isManualConfirmation ? 'Manual' : 'Automatically';
    _repeatType = widget.payment.repeatType;
    _selectedPaymentType = widget.payment.paymentType;
    _payeeName = widget.payment.payee;
    _note = widget.payment.note;
    _selectedDate = widget.payment.startDate;
  }
          ...
          Container(
            ...
            child: ElevatedButton(
              ...
              child: _isLoading 
                ...
                : const Text('Update', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
            ),
          ),
        ],
      ),
  ...
  Future<void> _updatePayment() async {
    ...
    try {
      ...
      final updatedPayment = PlannedPaymentModel(
        ...
        selectedDays: widget.payment.selectedDays, // Keep existing selected days
        isManualConfirmation: _confirmationType == 'Manual',
        currency: _selectedCurrency,
        isActive: true,
      );

      await firestoreService.updatePlannedPayment(updatedPayment);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Planned payment updated successfully!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error updating planned payment: ${e.toString()}'),
